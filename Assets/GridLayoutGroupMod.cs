...
namespace UnityEngine.UI
{
    ...
    public class GridLayoutGroupMod : LayoutGroup
    {
        ...
        private void SetCellsAlongAxis(int axis)
        {
            ...
            for (int i = 0; i < rectChildren.Count; i++)
            {
				...
				float realSizeX = ((width - (spacing [0] * (actualCellCountX - 1))) / actualCellCountX);
				SetChildAlongAxis(rectChildren[i], 0, startOffset.x + (cellSize[0] + spacing[0]) * positionX, realSizeX);
                //SetChildAlongAxis(rectChildren[i], 0, startOffset.x + (cellSize[0] + spacing[0]) * positionX, cellSize[0]);
				float realSizeY = ((height - (spacing [1] * (actualCellCountY - 1))) / actualCellCountY);
				SetChildAlongAxis(rectChildren[i], 1, startOffset.y + (cellSize[1] + spacing[1]) * positionY, realSizeY);
